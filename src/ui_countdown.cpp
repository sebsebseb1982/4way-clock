...
// ─────────────────────────────────────────────────────────────────────────────
// Helper: format g_countdown_ms into HH:MM:SS
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// Helper: create a small button with an event user_data
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// Build countdown UI — landscape 320×240 (rotation=3)
//
// Layout:
//   y=10-70   : label HH:MM:SS (centré)
//   y=80-120  : [+H] [+M] [+S]
//   y=130-170 : [-H] [-M] [-S]
//   y=180-215 : [START/PAUSE]    [RESET]
// ─────────────────────────────────────────────────────────────────────────────
void ui_countdown_build() {
    ...
    lv_obj_set_style_text_color(lbl_title, lv_color_make(120, 120, 120), LV_PART_MAIN);
    lv_label_set_text(lbl_title, "COMPTE A REBOURS");
    lv_obj_set_pos(lbl_title, 0, 8);
    lv_obj_set_width(lbl_title, 320);
    ...
    format_countdown(buf, sizeof(buf));
    lv_label_set_text(s_lbl_countdown, buf);
    lv_obj_set_pos(s_lbl_countdown, 0, 28);
    lv_obj_set_width(s_lbl_countdown, 320);
    lv_obj_set_style_text_align(s_lbl_countdown, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN);

    // Button columns: x positions for H, M, S columns
    const lv_coord_t col_x[3] = { 10, 115, 220 };
    const lv_coord_t btn_w    = 90;
    const lv_coord_t btn_h    = 38;

    // Row 1 : +H, +M, +S
    make_btn(screen, "+1h", col_x[0], 80, btn_w, btn_h, cb_adj, (void*)( 3600000));
    make_btn(screen, "+1m", col_x[1], 80, btn_w, btn_h, cb_adj, (void*)(   60000));
    make_btn(screen, "+1s", col_x[2], 80, btn_w, btn_h, cb_adj, (void*)(    1000));

    // Row 2 : -H, -M, -S
    make_btn(screen, "-1h", col_x[0], 128, btn_w, btn_h, cb_adj, (void*)(-3600000));
    make_btn(screen, "-1m", col_x[1], 128, btn_w, btn_h, cb_adj, (void*)(  -60000));
    make_btn(screen, "-1s", col_x[2], 128, btn_w, btn_h, cb_adj, (void*)(   -1000));

    // START / PAUSE button
    s_btn_start = lv_btn_create(screen);
    lv_obj_set_size(s_btn_start, 148, 40);
    lv_obj_set_pos(s_btn_start, 10, 182);
    ...
    lv_obj_set_style_radius(s_btn_start, 6, LV_PART_MAIN);
    ...
    lv_obj_set_size(btn_reset, 148, 40);
    lv_obj_set_pos(btn_reset, 162, 182);
